package jp.ac.osaka_u.cit_bach_01;
...
class Generator4 extends Generator {
	...
	private ResultOfGenerateOneTest generateOneTest(QuadTable tab,
				...
				for (int p2 = p1 + 1; p2 < numOfParameters - 1; p2++) {
					for (int p3 = p2 + 1; p3 < numOfParameters; p3++) {
						if (tab.get(p0, test.get(p0), p1, test.get(p1), p2,
								test.get(p2), p3, test.get(p3)) == false) {
							tab.set(p0, test.get(p0), p1, test.get(p1), p2,
									test.get(p2), p3, test.get(p3));
	...
	// 2.20
	// 前の位置をおぼえておく
	private void addUncoveredTuple(Testcase tmp, QuadTable tab,
							...
							for (byte v2 = 0; v2 < this.parametermodel.range[p2]; v2++) {
								...
								for (int p3 = p2 + 1; p3 < numOfParameters; p3++)
									for (byte v3 = 0; v3 < this.parametermodel.range[p3]; v3++) {
										if (tab.get(p0, v0, p1, v1, p2, v2, p3,
												v3) == false) {
											tmp.set(p0, v0);
											tmp.set(p1, v1);
											tmp.set(p2, v2);
	...
	private Testcase generateTempTest(Testcase seedrow, QuadTable tab,
			ArrayList<Integer>[] uncovTab) {
			...
			for (int p1 = p0 + 1; p1 < n - 2; p1++) {
				for (int p2 = p1 + 1; p2 < n - 1; p2++) {
					for (int p3 = p2 + 1; p3 < n; p3++) {
						...
						table[p0][p1][p2][p3] = new QuadList(
